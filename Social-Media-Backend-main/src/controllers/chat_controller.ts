...
export const InsertListChat = async ( uidSource: string, uidTarget: string )  => {
    ...
    try {
        ...
        if (verifyExistsSourceone[0][0].chat == 0){
            await conn.query('INSERT INTO list_chats (uid_list_chat, source_uid, target_uid) VALUES (?,?,?)', [ uuidv4(), uidSource, uidTarget ]);
        }
    } catch (error) {
        console.error('Error inserting list chat:', error);
        throw error;
    } finally {
        conn.end();
    }
...
export const addNewMessage = async ( uidSource: string, uidTarget: string, message: string ) => {
    const conn = await connect();
    try {
        await conn.query('INSERT INTO messages (uid_messages, source_uid, target_uid, message) VALUES (?,?,?,?)', [ uuidv4(), uidSource, uidTarget, message ]);
    } catch (error) {
